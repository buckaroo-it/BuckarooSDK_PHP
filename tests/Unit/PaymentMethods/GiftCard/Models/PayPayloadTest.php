...
class PayPayloadTest extends TestCase
{
    ...
    {
        $payload = new PayPayload(['servicesSelectableByClient' => 'ideal,creditcard']);

        $this->assertSame('ideal,creditcard', $payload->servicesSelectableByClient);
    }

    /** @test */
    public function it_sets_services_excluded_for_client(): void
    {
        $payload = new PayPayload(['servicesExcludedForClient' => 'paypal']);

        $this->assertSame('paypal', $payload->servicesExcludedForClient);
    }
    ...
    public function it_sets_all_properties_together(): void
    {
        $payload = new PayPayload([
            ...
            'servicesSelectableByClient' => 'ideal,bancontact',
            'servicesExcludedForClient' => 'sofort',
        ]);

        $this->assertSame('NoRedirect', $payload->continueOnIncomplete);
        $this->assertSame('ideal,bancontact', $payload->servicesSelectableByClient);
